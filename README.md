...
VTOP credentials and cached academic data are stored on the device. The app connects directly to VTOP. Google and Firebase connections are used only when the corresponding integration is configured or enabled.
...
Plain builds work without hosted integrations. Pass Dart defines through `.env.json` when developing the Gmail shared-login path or Firebase messaging and extension pairing.
